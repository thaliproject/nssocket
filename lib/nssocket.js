...
NsSocket.prototype.data = function (event, callback) {
  ...
  return this.on(['data'].concat(event), callback);
};
...
NsSocket.prototype.dataOnce = function (event, callback) {
  ...
  return this.once(['data'].concat(event), callback);
};
...
NsSocket.prototype.connect = function connect(/*port, host, callback*/) {
  ...
  this.connectCallback = this.connectCallback || callback;
  args = this.port ? [this.port, this.host] : [this.host];

  if (this.connectCallback) {
    args.push(this.connectCallback);
  }
...
NsSocket.prototype.reconnect = function reconnect() {
  ...
  if (
    this.retry.pending || // We already know that we should reconnect, we are waiting for timeout.
    this.retry.closed
  ) {
    return;
  }
  this.retry.pending = true;

  // Helper function containing the core reconnect logic
  function doReconnect() {
    // Cleanup and recreate the socket associated with this instance.
  ...
  function tryReconnect() {
    self.retry.pending = false;
    if (self.retry.closed) {
      return;
    }
  ...
  setTimeout(tryReconnect, this.retry.interval);
};
